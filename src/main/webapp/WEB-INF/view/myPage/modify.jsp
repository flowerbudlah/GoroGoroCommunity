						...
						<form id="modifyReportDTO" name="modifyReportDTO" enctype="multipart/form-data" style="margin-top: 50px; margin-bottom: 100px">
							...
							<div class="form-group">
								...
								<select id="reason" name="reason" class="form-control">
									<option value="명예훼손, 모욕, 비방, 허위사실 유포 등" <c:if test="${ReportDTOfromDB.reason eq '명예훼손, 모욕, 비방, 허위사실 유포 등'}">selected</c:if>>
										명예훼손, 모욕, 비방, 허위사실 유포 등 (Defamation, verbal abuse, slander, and the distribution of false information.)
									</option>
									<option value="광고, 도배 등" <c:if test="${ReportDTOfromDB.reason eq '광고, 도배 등'}">selected</c:if>>
										광고, 도배 등
									</option>
									<option value="음란물" <c:if test="${ReportDTOfromDB.reason eq '음란물'}">selected</c:if>>
										음란물
									</option>
									<option value="개인정보침해" <c:if test="${ReportDTOfromDB.reason eq '개인정보침해'}">selected</c:if>>
										개인정보침해
									</option>
									<option value="저작권침해" <c:if test="${ReportDTOfromDB.reason eq '저작권침해'}">selected</c:if>>
										저작권침해
									</option>
									<option value="기타(해당 게시판의 주제와 맞지않는내용 등)" <c:if test="${ReportDTOfromDB.reason eq '기타(해당 게시판의 주제와 맞지않는내용 등)'}">selected</c:if>>
										기타(해당 게시판의 주제와 맞지않는내용 등)
									</option>
								</select>
							</div>
							<div class="form-group">
